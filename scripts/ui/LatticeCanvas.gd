...
func _nearest_point(position: Vector2) -> int:
	...
	for index in range(points.size()):
		var distance = position.distance_to(_lattice_to_screen(points[index]))
...
func _screen_to_lattice(position: Vector2) -> Dictionary:
	var bounds: Dictionary = variation.get("bounds", {"x": 0.0, "y": 0.0, "width": 1.0, "height": 1.0})
	var rect = _canvas_rect()
	var u = clampf((position.x - rect.position.x) / maxf(1.0, rect.size.x), 0.0, 1.0)
	var v = clampf((position.y - rect.position.y) / maxf(1.0, rect.size.y), 0.0, 1.0)
	return {
